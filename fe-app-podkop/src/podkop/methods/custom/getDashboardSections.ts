...
export async function getDashboardSections(): Promise<IGetDashboardSectionsResponse> {
  ...
  const subscriptionLocationsBySection = new Map<string, Record<string, string>>();
  await Promise.all(
    configSections
      .filter(
        (section) =>
      ...
      .map(async (section) => {
        const response = await PodkopShellMethods.getSubscriptionLocations(
          section['.name'],
        );
        if (response.success) {
          subscriptionLocationsBySection.set(section['.name'], response.data);
        }
      }),
  );
    ...
    .map((section) => {
      if (section.connection_type === 'proxy') {
        ...
        if (section.proxy_config_type === 'subscription') {
          ...
          const locations =
            subscriptionLocationsBySection.get(section['.name']) ?? {};
          const locationTagPattern = new RegExp(
            `^${section['.name']}-(loc\\d+)-urltest-out$`,
          );

          const locationTags = selector?.value?.all ?? [];

          const outbounds = locationTags.map((code) => {
            const item = proxies.find((proxy) => proxy.code === code);
            const slug = code.match(locationTagPattern)?.[1];
            const displayName =
              (slug && locations[slug]) || item?.value?.name || code;
